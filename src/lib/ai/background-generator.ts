...
export async function checkAndTriggerBackgroundGeneration(
  ...
  try {
    ...
    if (incorrectQuestionIds.length > 0) {
      console.log(`[BackgroundGen] ${incorrectQuestionIds.length} yanlış soru için takip soruları planlandı`);
    ...
    if ((existingCount || 0) < antrenmanQuota) {
      console.log(`[BackgroundGen] Kota dolmamış (${existingCount}/${antrenmanQuota}), arka plan üretimi tetikleniyor...`);
      
      // Trigger Edge Function in background (fire-and-forget)
      supabase.functions.invoke('quiz-generator', {
        body: { chunkId, courseId }
      }).catch(err => {
        console.warn('[BackgroundGen] Edge Function trigger failed:', err);
      });
    }
  } catch (e) {
