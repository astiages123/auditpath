...
import { ACHIEVEMENTS, getRequirementDescription } from "@/lib/achievements";
import { getRankIcon, RANKS } from "@/lib/rank-icons";
...
export function getCelebrationAsset(id: string): CelebrationAsset {
  ...
  if (id.startsWith("RANK_UP:")) {
    ...
    const rank = RANKS.find(r => r.id === rankId || r.name === rankId);
