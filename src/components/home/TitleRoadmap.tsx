...
import { Sparkles, Trophy, Lock, CheckCircle2, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { motion } from "framer-motion";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { RANKS } from "@/lib/rank-icons";
...
export default function TitleRoadmap({ watchedVideos, requiredVideos }: TitleRoadmapProps) {
    ...
    const progress = requiredVideos > 0 ? Math.min(Math.round((watchedVideos / requiredVideos) * 100), 100) : 0;

    let currentRankIndex = milestones.length - 1;
    for (let i = 0; i < milestones.length - 1; i++) {
        if (progress >= milestones[i].threshold && progress < milestones[i + 1].threshold) {
            currentRankIndex = i;
            break;
        }
    }
        ...
        <Card className="mx-auto w-full max-w-4xl border-primary/10 bg-background/95 backdrop-blur-xl shadow-2xl overflow-hidden relative ring-1 ring-border/50">
            ...
            <CardHeader className="relative flex flex-row items-center justify-between py-4 px-6 border-b border-border/40">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <CardTitle className="text-xl font-bold tracking-tight">Unvan Yolculuğu</CardTitle>
                        <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wider">
                            Seviye: <span className="text-primary">{currentMilestone.title}</span>
                        </p>
                    </div>
                </div>
                <div className="flex items-baseline gap-1 bg-muted/40 px-3 py-1 rounded-xl border border-border/50">
                    <span className="text-2xl font-black font-mono text-primary leading-none">{progress}</span>
                        ...
                        {[0, 25, 50, 75, 100].map((tickValue) => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{ left: `${positionLeft}%`, transform: 'translate(-50%, -50%)' }}
                                >
                                    <div className={cn(
                                        "w-6 h-6 rounded-full border-4 transition-all duration-500 flex items-center justify-center z-10 bg-background",
                                        isPassed ? "border-primary text-primary" : "border-white/20 text-muted-foreground"
                                    )}>
                                        {isPassed ? <CheckCircle2 className="w-3.5 h-3.5" /> : <div className="w-1.5 h-1.5 rounded-full bg-current opacity-40" />}
                                    </div>
                                    <span className="absolute top-7 text-[10.5px] font-black font-foreground font-mono">%{tickValue}</span>
                                </div>
                            );
                        })}
                    </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <h3 className={cn("text-sm font-bold truncate px-1", isCurrent ? "text-primary" : "text-foreground")}>
                                        {milestone.title}
                                    </h3>
                    ...
                    {nextMilestone ? (
                        <div className="bg-muted/30 rounded-xl p-3 flex items-center justify-between border border-border/30">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-semibold text-muted-foreground">Sıradaki: <span className="text-foreground">{nextMilestone.title}</span></span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-lg">%{nextMilestone.threshold} Gerekiyor</span>
