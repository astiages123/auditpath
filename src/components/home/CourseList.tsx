...
import { CourseStatsModal } from "../courses/CourseStatsModal";
...
export function CourseList({
  ...
  // Celebration Hook
  const { triggerCourseCelebration, revokeCourseCelebration } = useCelebration();
  ...
  }, [courses, stats.courseProgress, triggerCourseCelebration, revokeCourseCelebration]);
    ...
    <div className="space-y-3">
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center p-4 gap-4">
              ...
              >
                {/* Icon */}
                <div className={`p-3 rounded-xl shrink-0 transition-transform duration-500 group-hover:scale-110 ${isCompleted ? "bg-amber-500/20 text-amber-400 ring-1 ring-amber-500/30" : `${categoryBgColor} ${categoryColor}`}`}>
                  <Icon className={`h-6 w-6 ${isCompleted ? "text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]" : categoryColor}`} />
                </div>

                {/* Info */}
                <div className="flex flex-col gap-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className={`font-bold text-base truncate transition-colors ${isCompleted ? "text-amber-200" : "text-zinc-100"}`}>
                      {displayName}
                    </h4>
                    <span className={`text-sm font-bold ${isCompleted ? "text-amber-400" : categoryColor}`}>
                      %{progress}
                    </span>
                    {isCompleted && (
      ...
      {selectedStatsCourse && (
        <CourseStatsModal
          ...
          onOpenChange={(open) => !open && setSelectedStatsCourse(null)}
      ...
      {selectedQuizCourse && (
        <QuizModal
          ...
          onOpenChange={(open) => !open && setSelectedQuizCourse(null)}
