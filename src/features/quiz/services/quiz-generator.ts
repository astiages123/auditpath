...
import { generateConceptMap, type ConceptMapItem } from '../modules/ai/mapping';
import { generateQuestionBatch } from '../modules/ai/question-generation';
import { validateQuestionBatch, type QuestionToValidate } from '../modules/ai/validation';
  ...
  try {
    ...
    if (chunkData.metadata?.concept_map && chunkData.metadata.concept_map.length > 0) {
      ...
      concepts = await generateConceptMap(
        ...
        (msg: string, details?: Record<string, unknown>) => log('MAPPING', msg, (details || {}) as Record<string, unknown>)
      );
    ...
    const guidelines = await subjectKnowledgeService.getGuidelines(chunkData.course_name);
    ...
    while (totalGenerated < remaining && conceptIndex < concepts.length) {
      ...
      const generated = await generateQuestionBatch(
        ...
        conceptIndex,
        guidelines,
        (msg: string, details?: Record<string, unknown>) => log('GENERATING', msg, (details || {}) as Record<string, unknown>)
      );
      ...
      const validationResults = await validateQuestionBatch(
        ...
        chunkData.content,
        (msg: string, details?: Record<string, unknown>) => log('VALIDATING', msg, (details || {}) as Record<string, unknown>)
      );
...
export async function generateFollowUpSingle(
  ...
  try {
    ...
    // 2. Fetch guidelines
    const guidelines = await subjectKnowledgeService.getGuidelines(chunk.course_name);

    // 3. Generate question (no validation stage for follow-ups to keep it fast/adaptive)
    const question = await import('../modules/ai/question-generation').then(m => 
      m.generateFollowUpQuestion(
        context,
        chunk.course_name,
        chunk.section_title,
        guidelines,
        (msg: string, details?: Record<string, unknown>) => onLog(msg, details)
      )
    );
