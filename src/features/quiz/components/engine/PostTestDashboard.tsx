...
export function PostTestDashboard({ results, onClose, courseName }: PostTestDashboardProps) {
                        ...
                        <svg className="w-full h-full transform -rotate-90">
                            ...
                            <circle
                                ...
                                className={`fill-none transition-all duration-1000 ease-out ${
                                    results.percentage >= 70 ? 'stroke-emerald-500' : 'stroke-amber-500'
                                }`}
                                strokeWidth="12"
                                strokeDasharray={553}
                        ...
                        <div className="p-4 bg-primary/5 rounded-xl border border-primary/20 space-y-3">
                            <div className="flex items-center gap-2 text-primary">
                                ...
                                <span className="font-bold uppercase tracking-wider text-xs">
                                    {strategy.importance === 'high' ? 'Kritik Ders Analizi' : 'Konu Değerlendirmesi'}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center gap-4 pt-2 border-t border-white/5">
                                <div className="flex items-center gap-1.5">
                                    <Star className={`w-3.5 h-3.5 ${strategy.importance === 'high' ? 'text-amber-500 fill-amber-500' : 'text-zinc-500'}`} />
                                    <span className="text-[10px] font-bold text-zinc-400 uppercase">Önem: {strategy.importance.toUpperCase()}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <AlertTriangle className="w-3.5 h-3.5 text-zinc-500" />
                                    <span className="text-[10px] font-bold text-zinc-400 uppercase">Soru Payı: %{Math.round((strategy.examTotal / 120) * 100)}</span>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {!strategy && (
                        <div className="text-muted-foreground">
                            {results.pendingReview > 0 ? (
                                <p>
                                    <span className="text-amber-500 font-medium">{results.pendingReview} soru</span> tekrar listesine eklendi. 
                                    AI asistanın, yanlış yapılan sorular için özel takip soruları hazırlıyor...
                                </p>
                            ) : (
                                <p>
                                    <span className="text-emerald-500 font-medium">Mükemmel!</span> Hiçbir soru tekrar listesine düşmedi. 
                                    Konu hakimiyetin artıyor.
                                </p>
                            )}
                        </div>
                    )}
