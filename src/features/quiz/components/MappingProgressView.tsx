      ...
      <div className="w-full max-w-md space-y-10 py-6 mx-auto my-auto">
        ...
        <div className="space-y-8 relative">
          ...
          {steps.map((step, i) => {
            ...
            const stepActive =
              !hasError && i === currentStepIndex && !isAllDone;
            ...
            return (
              <div key={step.id} className="relative flex flex-col gap-1.5">
                ...
                {displayMessage && (
                  <div className="ml-12 text-[13px] text-muted-foreground/70 animate-in fade-in slide-in-from-top-1 flex flex-col gap-1.5">
                    <span>{displayMessage}</span>
                    {step.id === 'design' && examProgress.total > 0 && (
                      <span className="text-primary font-medium tracking-tight">
                        Oluşturulan: {examProgress.current} soru /{' '}
                        {examProgress.total} kota
                      </span>
                    )}
                  </div>
                )}
