...
interface CourseOverviewProps {
  /** Dersin tam adı */
  courseName: string;
  /** Dersin genel ilerleme verileri */
  progress: {
    total: number;
    solved: number;
    percentage: number;
...
interface QuotaDisplayProps {
  ...
  loading: boolean;
  /** Bu oturumda kaydedilen soru sayısı */
  savedCount: number;
  /** Yüzdelik ilerleme (0-100) */
  progress: number;
  /** Mevcut tamamlama yüzdesi (0-100) */
  percentage: number;
  /** Mevcut üretim adımı etiketi */
  currentStep?: string;
  /** Tespit edilen kavram sayısı */
  conceptCount: number;
}

/**
 * Kullanıcıya soru kotasını, mevcut kullanımı ve üretim ilerlemesini görselleştiren bileşen.
 */
...
/**
 * Soru üretim sürecindeki teknik adımları canlı olarak listeleyen bölüm.
 */
export const GenerationLiveStream: FC<GenerationLiveStreamProps> = ({
  ...
  return (
    <div className="flex-1 overflow-hidden border border-border/50 rounded-lg bg-muted/20">
      {/* BAŞLIK */}
      <div className="p-2 border-b border-border/30 bg-muted/30 flex justify-between items-center">
