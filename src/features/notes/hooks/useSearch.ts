...
export function useSearch({
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    while ((match = searchRegex.exec(content)) !== null) {
      ...
      const contentBefore: string = content.substring(0, position);
      const contentAfter: string = content.substring(
        position + matchTextOriginal.length
      );
      ...
      const lineEnd: number =
        firstNewlineAfter === -1
          ? content.length
          : position + matchTextOriginal.length + firstNewlineAfter;

      const beforeContent: string = content.substring(lineStart, position);
