...
import { calculateAchievements, ACHIEVEMENTS } from "@/shared/lib/domain/achievements";
import { RANKS, type Rank } from "@/config/constants"; 
import { ProgressStats } from "@/shared/hooks/useProgress";
import coursesData from "@/features/courses/data/courses.json";

export const achievementKeys = {
    all: ["achievements"] as const,
    uncelebrated: (userId: string) => [...achievementKeys.all, "uncelebrated", userId] as const,
};
...
async function syncAchievements({ stats, userId }: SyncContext) {
    ...
    if (dailyError) {
         ...
         const isAbort = dailyError.message?.includes("AbortError") || dailyError.code === "ABORT_ERROR";
         if (!isAbort) console.error("Failed to fetch daily video count:", dailyError);
    }
    ...
    const currentRankOrder = (RANKS as Rank[]).find((r: Rank) => r.id === currentRankId)?.order ?? -1;
    ...
    // 3. Course/Category
    (coursesData as Array<{ category: string; courses: Array<{ id: string; totalVideos: number }> }>).forEach(cat => {
    ...
    if (toUnlock.length > 0) {
        ...
        await supabase.from("user_achievements").upsert(updates, { onConflict: "user_id,achievement_id", ignoreDuplicates: true });
    }
    ...
    if (!isIncompleteLoad) {
            ...
            const knownPrefixes = ["RANK_UP:", "COURSE_COMPLETION:", "CATEGORY_COMPLETION:"];
            
            const toRevoke = [...dbIds].filter(id => {
                const isManaged = knownPrefixes.some(p => id.startsWith(p)) || staticIds.includes(id); 
    ...
    return useMutation({
        ...
        onSuccess: (hasNewUnlocks, variables) => {
            if (hasNewUnlocks) {
                ...
                queryClient.invalidateQueries({ queryKey: achievementKeys.uncelebrated(variables.userId) });
            }
        },
        onError: (err) => {
             // Safe error logging
             const isAbort = (err instanceof Error && (err.name === 'AbortError' || err.message?.includes('AbortError')));
             if (!isAbort) console.error("Achievement Sync Mutation Error:", err);
        }
    });
}
