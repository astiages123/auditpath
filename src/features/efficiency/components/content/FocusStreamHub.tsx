...
import { DistractionDetails } from '@/features/efficiency/components/content/DistractionDetails';
...
const EfficiencyTrendChart = lazy(() =>
  import('@/features/efficiency/components/charts/EfficiencyTrendChart').then(
    (m) => ({
...
export const FocusStreamHub = ({
  ...
  const isHistoryEmpty = trendData.length === 0;
  const isAnalysisEmpty = sessions.length === 0;
  ...
  return (
    <Tabs defaultValue="analysis" className="w-full">
      ...
      >
        {isAnalysisEmpty ? (
          <CommonEmptyState message="Analiz edilecek çalışma verisi bulunamadı." />
        ) : (
      ...
      >
        <div className="space-y-4">
          ...
          {isHistoryEmpty ? (
            <CommonEmptyState message="Son 30 güne ait akış verisi bulunamadı." />
          ) : (
