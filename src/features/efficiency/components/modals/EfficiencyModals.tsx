...
const DistractionAnalysis = ({ sessions }: { sessions: Session[] }) => {
    ...
    const totalPauseMinutes = sessions.reduce((acc, s) => {
        const pauseEvents = s.timeline?.filter(t => t.type === 'pause') || [];
        const pauseMins = pauseEvents.reduce((pAcc, p) => pAcc + (p.duration || 0), 0);
        return acc + pauseMins;
    }, 0);

    const totalBreakMinutes = sessions.reduce((acc, s) => {
        const breakEvents = s.timeline?.filter(t => t.type === 'break') || [];
        const breakMins = breakEvents.reduce((pAcc, p) => pAcc + (p.duration || 0), 0);
    ...
    const allPauses = sessions.flatMap(s => 
        (s.timeline?.filter(t => t.type === 'pause') || []).map(p => ({
            ...p,
            lessonName: s.lessonName,
            timeLabel: new Date(p.start).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })
        }))
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col items-center text-center">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Odak Gücü</span>
                    <span className={cn("text-2xl font-black font-heading", getStabilityColor(focusPower))}>
                        {focusPower} <span className="text-xs font-medium opacity-50">puan</span>
                    </span>
                    <span className="text-[11px] text-muted-foreground mt-1">Oturum boyunca ne kadar bölünmeden ve mola disipliniyle çalıştığınızı ölçer.</span>
                </div>
                ...
                <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col items-center">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Toplam Duraklatma</span>
                    <span className="text-2xl font-black font-heading text-white">
                        {totalPauses} <span className="text-sm font-medium text-muted-foreground">kez</span>
                    </span>
                    <span className="text-[11px] text-muted-foreground mt-1">Bugünkü oturumlar genelinde</span>
                </div>
                ...
                <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col items-center">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Kayıp Zaman</span>
                    <span className="text-2xl font-black font-heading text-white">
                        {totalPauseMinutes} <span className="text-sm font-medium text-muted-foreground">dk</span>
                    </span>
                    <span className="text-[11px] text-muted-foreground mt-1">Duraklatma süresi</span>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-2">
                    ...
                    <h4 className="text-sm font-bold text-white uppercase tracking-wider">Duraklatma Günlüğü</h4>
                </div>

                {allPauses.length > 0 ? (
                    <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {allPauses.map((pause, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3.5 bg-white/3 border border-white/5 rounded-xl hover:bg-white/6 transition-all">
                                <div className="flex items-center gap-4">
                                    <div className="w-1.5 h-1.5 rounded-full bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]" />
                                    <div className="flex flex-col">
                                        <span className="text-xs font-bold text-white/90">{pause.lessonName}</span>
                                        <span className="text-[10px] text-muted-foreground">{pause.timeLabel} civarında</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-xs font-mono font-bold text-rose-400">{pause.duration} dk</span>
                    ...
                    <div className="p-12 text-center rounded-2xl bg-white/2 border border-dashed border-white/10">
                        ...
                        <p className="text-sm text-muted-foreground">Bugün henüz bir duraklatma kaydedilmedi.</p>
                        <p className="text-[11px] text-muted-foreground/40 mt-1">Kesintisiz odaklanmaya devam ediyorsunuz!</p>
                    </div>
                )}
            </div>
        </div>
...
export const FocusHubContent = ({ sessions, trendData }: { sessions: Session[]; trendData: EfficiencyTrend[] }) => (
        ...
        <TabsContent value="history" className="p-4 border border-border/50 rounded-xl mt-4 bg-white/5">
            <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                    <h4 className="text-sm font-bold text-white uppercase tracking-wider">Günlük Öğrenme Akışı Trendi</h4>
                    <span className="text-[10px] text-muted-foreground">SON 30 GÜN</span>
                </div>
                <EfficiencyTrendChart data={trendData} />
                <p className="text-xs text-muted-foreground text-center">
                    Öğrenme akışı; video içeriği ile harcadığınız çalışma süresi arasındaki uyumu ve öğrenme ritmini ölçer.
                </p>
            </div>
        </TabsContent>
    </Tabs>
...
export const LearningLoadContent = ({ dayData, weekData, monthData, allData, targetMinutes = 200 }: LearningLoadContentProps) => (
