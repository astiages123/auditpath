...
export const CognitiveInsightsCard = () => {
    const { loading, cognitiveAnalysis } = useEfficiencyData();
        ...
        return (
            <GlassCard className="h-full flex items-center justify-center p-6 min-h-[300px]">
                <div className="animate-pulse flex flex-col items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground/50">Analizler yükleniyor...</span>
                </div>
            </GlassCard>
        );
    }

    const { focusScore, topConfused, recentInsights, criticalTopics, hasData } = cognitiveAnalysis || {};
            ...
            <GlassCard className="h-full flex flex-col p-6 min-h-[300px]">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 pt-4">
                    ...
                    <div className="space-y-1.5 max-w-xs">
                        <h3 className="text-base font-medium text-white/90">Zekanı Konuştur!</h3>
                        <p className="text-sm text-muted-foreground/60 leading-relaxed">
                            İlk quizlerini tamamladığında yapay zeka destekli bilişsel analizlerin burada belirecek.
                        </p>
                    </div>
                </div>
            </GlassCard>
        ...
        <GlassCard className="h-full flex flex-col p-6 min-h-[300px]">
             <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex flex-col items-end">
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold mb-0.5">Odak Puanı</div>
                    <div className={cn(
                        "text-2xl font-black font-heading",
                        (focusScore || 0) >= 80 ? "text-emerald-400" : 
                        (focusScore || 0) >= 60 ? "text-amber-400" : "text-rose-400"
                    )}>
                        {focusScore}
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            {criticalTopics.map((topic) => (
                                <div key={topic.id} className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 flex gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-white/90 line-clamp-2">
                                            {topic.diagnosis || "Bu konuda zorlandığınız tespit edildi. Destek mekanizması devrede."}
                                        </p>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            {topConfused.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between p-2.5 rounded-lg bg-white/3 border border-white/5 hover:bg-white/5 transition-colors">
                                    <span className="text-sm text-white/80 line-clamp-1">{item.text}</span>
                                    {item.count > 1 && (
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            {recentInsights.map((insight, idx) => (
                                <div key={idx} className="p-3 rounded-lg bg-sky-500/5 border border-sky-500/10 text-sm text-sky-100/80 leading-relaxed italic">
                                    "{insight}"
                                </div>
                            ))}
                        </div>
