                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {schedule.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <div className={cn("absolute top-0 right-0 w-16 h-16 rounded-full blur-2xl opacity-10 -mr-8 -mt-8 pointer-events-none", item.color.replace('text-', 'bg-'))} />

                                <CardContent className="p-3">
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="space-y-0.5">
                                            <h3 className={cn("text-sm font-semibold", isToday ? "text-primary" : "text-muted-foreground")}>
                                                {item.day}
                                            </h3>
                                            {isToday && (
                                                <Badge variant="secondary" className="bg-primary/10 text-primary text-[9px] h-4 px-1.5 flex w-fit">
                                                    BUGÜN
                                                </Badge>
                                            )}
                                        </div>
                                        <div className={cn("p-1.5 rounded-md bg-background shadow-sm border border-border/50", item.color)}>
                                ...
                                <div className={cn("h-0.5 w-full opacity-50", item.bg.replace('/10', '/50'))} />
                            </Card>
                        );
                    })}
                </div>
