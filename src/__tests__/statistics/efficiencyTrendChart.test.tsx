...
import { StatisticsTrendChart } from '@/features/statistics/components/charts/StatisticsTrendChart';
...
describe('StatisticsTrendChart - UI Test', () => {
  it('should process { score: 85 } into { score: 85, deviation: 84 } and pass it to BarChart', async () => {
    ...
    const { findByTestId } = render(<StatisticsTrendChart data={mockData} />);
  ...
  it('should set the Y-Axis domain correctly, not bound to [0,0]', async () => {
    const { findByTestId } = render(<StatisticsTrendChart data={[]} />);
