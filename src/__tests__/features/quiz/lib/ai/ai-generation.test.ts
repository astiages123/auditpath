...
import { buildDraftingTaskPrompt } from '@/features/quiz/lib/ai/prompts';
import { ValidationTask } from '@/features/quiz/lib/ai/tasks/validation-task';
import { StructuredGenerator } from '@/features/quiz/lib/ai/utils';
import type { ConceptMapItem } from '@/features/quiz/core/types';

// Mock StructuredGenerator
vi.mock('@/features/quiz/lib/ai/utils', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/features/quiz/lib/ai/utils')>();
  return {
    ...actual,
    StructuredGenerator: {
      generate: vi.fn(),
