...
import type { ValidationResult } from '@/features/quiz/lib/ai/schemas';
...
vi.mock('@/features/quiz/lib/ai/tasks/analysis-task', () => ({
  AnalysisTask: class {
    run = mockAnalysisTask.run;
  },
}));

vi.mock('@/features/quiz/lib/ai/tasks/drafting-task', () => ({
  DraftingTask: class {
    run = mockDraftingTask.run;
  },
}));

vi.mock('@/features/quiz/lib/ai/tasks/validation-task', () => ({
  ValidationTask: class {
    run = mockValidationTask.run;
  },
}));

vi.mock('@/features/quiz/lib/ai/tasks/revision-task', () => ({
  RevisionTask: class {
    run = mockRevisionTask.run;
  },
}));

vi.mock('@/features/quiz/lib/ai/tasks/follow-up-task', () => ({
  FollowUpTask: class {
