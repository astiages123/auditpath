...
const SUPABASE_KEY = process.env.VITE_SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
    ...
    for (const catData of coursesData) {
        ...
        for (let i = 0; i < catData.courses.length; i++) {
            ...
            if (existingCourse) {
                ...
                await supabase.from('Course').update({
                ...
                if (createCourseError) {
                    console.error(`Error creating course ${courseData.name}:`, createCourseError);
                    continue;
                }
        ...
        await supabase.from('Category').update({ totalHours: categoryTotalHours }).eq('id', categoryId);
    }
