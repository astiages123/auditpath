...
- **React Hooks**: Follow the Rules of Hooks stricty. Exhaustive deps must be respected. **Avoid `eslint-disable` for hook dependencies.**
...
- `fix: ...` for bug fixes.
- `chore: ...` for maintenance, config changes, dependency updates.
- `docs: ...` for documentation.
- `style: ...` for formatting, missing semi colons, etc (no code change).
- `refactor: ...` for refactoring production code.
- `test: ...` for adding/refactoring tests.
...
- Only expand output if explicitly requested by the user.

## 7. Output Optimization
- Use short, structured responses.
- Split large outputs into parts, if necessary, asking user before continuing.
- Prefer minimal formats (JSON, CSV, bullet points) to reduce tokens.
